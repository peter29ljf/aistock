# aistock — Claude Code 项目上下文

AI 自主交易控制台。FastAPI 后端 + React 前端，每条策略对应一个 Claude Code agent，通过 MCP 工具与 Tiger Brokers 真实账户交互。

## 安全约束（最高优先级）

- Tiger 账户是 **真实实盘（env=PROD）**，资金操作不可逆
- Claude Code 开发辅助时**严禁直接下单**，下单只能通过策略 AI agent 执行
...
backend/app/
  ...
  claude_runner.py   以 stream-json 调起 claude CLI
...
- Claude Code CLI: `~/.local/bin/claude`（可通过 `AISTOCK_CLAUDE_BIN` 覆盖）
