...
mcp_servers/       MCP tool servers (stdio, one process per strategy session)
...
### yfinance-tools (bundled)

| Tool | Description |
|---|---|
| `get_stock_quote` | Current price for any session — pre-market, regular, post-market, overnight |
| `get_klines` | OHLCV K-lines (1m / 5m / 15m / 1h / 1d …) |
| `get_indicators` | MA, RSI, Bollinger Bands, KDJ |

### tiger-openapi (separate repo: [peter29ljf/tigermcp](https://github.com/peter29ljf/tigermcp))

| Tool | Description |
|---|---|
| `get_option_expirations` | List option expiration dates for a symbol |
| `get_option_chain` | Full option chain with OI / IV / Greeks |
| `get_oi_analysis` | OI support/resistance levels, Max Pain, PCR |
| `get_market_status` | Current market session (PRE / REGULAR / POST / CLOSED) |
| `get_account` | Account summary |
| `get_positions` | Open positions |
| `get_assets` | Asset breakdown |
| `get_orders` | Order history |
| `place_order` | Place limit / market / stop / trailing-stop; supports `outside_rth` and `trading_session_type` (RTH / PRE_RTH_POST / OVERNIGHT / FULL) |
| `cancel_order` | Cancel an open order |

### portfolio / strategy-doc / price-alert / scheduler (bundled)

CRUD tools for per-strategy portfolio records, strategy documents, price alerts, and cron tasks. Called by the agent to persist state without touching files directly.
...
Clone and configure the Tiger MCP server:

```bash
git clone https://github.com/peter29ljf/tigermcp.git
pip install tigeropen mcp
```

Update the path to `server.py` in your strategy `.mcp.json` (auto-generated by the backend, see step 4).
...
### 4. Internal Token (auto-generated)

On first startup the backend writes a random token to `data/internal_token`. This token authenticates MCP servers calling back into the backend. The file is excluded by `.gitignore` and **must not be committed**.

### 5. Strategy `.mcp.json` (auto-generated)
...
- [peter29ljf/tigermcp](https://github.com/peter29ljf/tigermcp) — Tiger Brokers MCP server
