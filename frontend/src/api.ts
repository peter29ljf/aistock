...
export const api = {
  ...
  getMcpHealth: () => fetch("/api/health/mcp").then(j<Record<string, { status: string; detail?: string; jobs?: number; mode?: string }>>),
  getTigerLive: () => fetch("/api/health/mcp/tiger_live").then(j<{ status: string; detail?: string; mode?: string }>),
